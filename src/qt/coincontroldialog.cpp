...
{
    GUIUtil::setClipboard(DL0GUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    DL0GUnit nDisplayUnit = DL0GUnit::BTC;
    ...
    l2->setText(DL0GUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(DL0GUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(DL0GUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(DL0GUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    DL0GUnit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, DL0GUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, DL0GUnits::format(nDisplayUnit, nSum));
