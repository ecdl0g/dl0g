...
{
    //Starting value
    CAmount nSubsidy = 100ULL * COIN;

    //Compute what epoch we are in
    int64_t block_count_in_epoch = consensusParams.nPowTargetTimespan/consensusParams.nPowTargetSpacing ;
    int64_t epoch_count          = (int64_t)nHeight / block_count_in_epoch;
  
    if (epoch_count > 338) return 157 * COIN / 50; //After epoch 400 return Pi coins as rewards, 3.14.

    /////////////////////////////////////////////////////////////////
    //Multiply by ~90% for every new odd epoch on the chain        //
    //Note 2: The supply at epoch 400 will be no more than         //
    //        21,708,717.2 dl0g coins.                             //
    //        After that, 3.14 coins per block reward. Pi is nice. //
    /////////////////////////////////////////////////////////////////
    //
    //  Here's the total supply table for the curious.
    //
    //  ---------------------------------------------------------------------
    //  -    Epoch Ends         Block     Total Supply    Block Reward           
    //  -        13             26208       2620800          100
    //  -        26             52416       4953312           89
    //  -        39             78624       7023744           79
    //  -        52            104832       8858304           70
    //  -        65            131040      10483200           62
    //  -        78            157248      11924640           55
    //  -        91            183456      13208832           49
    //  -       104            209664      14335776           43
    //  -       117            235872      15331680           38
    //  -       130            262080      16196544           33
    //  -       143            288288      16956576           29
    //  -       156            314496      17611776           25
    //  -       169            340704      18188352           22
    //  -       182            366912      18686304           19
    //  -       195            393120      19105632           16
    //  -       208            419328      19472544           14
    //  -       221            445536      19787040           12
    //  -       234            471744      20049120           10
    //  -       247            497952      20258784            8
    //  -       260            524160      20442240            7
    //  -       273            550368      20599488            6
    //  -       286            576576      20730528            5
    //  -       299            602784      20835360            4
    //  -       312            628992      20913984            3
    //  -       325            655200      20966400            2
    //  -       338            681408      20992608            1
    //  --------------------------------------------------------------------
    // 
    //  After this, 3.14 DL0G coins per block reward or 82,293.12 
    //  DL0G coins per 13 epochs or 3 months or every quarter.
    //  Yearly that is 329,172.48 DLOG coins, minus the burned coins
    //  from the discrete logarithm as a service that will be 
    //  implemented later.
    ////////////////////////////////////////////////////////////
    
    for( int64_t p = 1; p < epoch_count; p++){
        if (p%13 == 0  ){ //Roughly every 4 months at target block time speed -- or 13 epochs.
            nSubsidy *= 229ULL;    //~90% coin reward retention per epoch, higher than bitcoin's halving.
            nSubsidy = nSubsidy >> 8;
        }
    }    
  
    //Return the subsidy plus the constant reward of 3.14 coins.
    return nSubsidy  ;
}
...
void Chainstate::InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("%s: invalid block=%s  nbits=%d  height=%d  log2_work=%f  date=%s\n", __func__,
      pindexNew->GetBlockHash().ToString(), pindexNew->nBits,  pindexNew->nHeight,
      log(pindexNew->nChainWork.getdouble())/log(2.0), FormatISO8601DateTime(pindexNew->GetBlockTime()));
    CBlockIndex *tip = m_chain.Tip();
    assert (tip);
    LogPrintf("%s:  current best=%s  nbits=%d  height=%d  log2_work=%f  date=%s\n", __func__,
      tip->GetBlockHash().ToString(), tip->nBits,  m_chain.Height(), log(tip->nChainWork.getdouble())/log(2.0),
    ...
    const std::string& warning_messages) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    AssertLockHeld(::cs_main);
    LogPrintf("%s%s: new best=%s nBits=%d height=%d version=0x%08x log2_work=%f tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)%s\n",
        prefix, func_name,
        tip->GetBlockHash().ToString(), tip->nBits,  tip->nHeight, tip->nVersion,
