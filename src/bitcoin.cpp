...
Options:
  -m, --multiprocess     Run multiprocess binaries dlog-node, dlog-gui.
  -M, --monolithic       Run monolithic binaries dlogd, dlog-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show this help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'dlog-qt [ARGS]' or 'dlog-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'dlogd [ARGS]' or 'dlog-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'dlog-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'dlog-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'dlog-tx [ARGS]'.
...
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_dlog [ARGS]'.
  chainstate [ARGS] Run bitcoin kernel chainstate util, equivalent to running 'dlog-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_dlog [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_dlog-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(cmd.use_multiprocess ? "dlog-gui" : "dlog-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(cmd.use_multiprocess ? "dlog-node" : "dlogd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("dlog-cli");
            ...
            args.emplace_back("dlog-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("dlog-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_dlog");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("dlog-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_dlog");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_dlog-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("dlog-util");
        } else {
